...
int make_listener(int port) {
  ...
  s.sin_family = AF_INET; // socket internet family: Address Family Internet (IPv4)
  /*
...
void send_back_to_client(int cfd, ssize_t n, char *buf) {
  ...
  while (sent < n) {
    ssize_t s = send(
        cfd, // the destination is the client file descriptor
...
void handle_client(int cfd) {
  ...
  for (;;) {
    ...
    if (n > 0) send_back_to_client(cfd, n, buf); // echo server behaviour
    else if (n == 0) return; // client has closed the connection
    else if (n < 0) { perror("recv"); return; } // error
  }
}
